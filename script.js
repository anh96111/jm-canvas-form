...
// Create canvas item HTML with new order
function createCanvasItem(index) {
    ...
    canvasItem.innerHTML = `
        ...
        <div class="preview-section">
            <h3 data-translate="preview">Canvas Preview</h3>
    ...
    // Initialize preview for this canvas
    updateCanvasPreview(index);
}

// Handle image upload with multiple images
    ...
    Array.from(files).forEach(file => {
        ...
        reader.onload = function(e) {
            ...
            showCropModal(e.target.result, function(croppedImage) {
                ...
                updateCanvasPreview(canvasIndex);
                updateTotalPrice();
            });
        };
...
function updateImageThumbnails(canvasIndex) {
    ...
    images.forEach((image, index) => {
        ...
        if (index === canvasData[canvasIndex].selectedImageIndex) {
        ...
        thumbnailItem.onclick = function(e) {
            if (!e.target.classList.contains('delete-btn')) {
                selectImage(canvasIndex, index);
            }
        };
...
// Select image
function selectImage(canvasIndex, imageIndex) {
    canvasData[canvasIndex].selectedImageIndex = imageIndex;
    updateImageThumbnails(canvasIndex);
    updateCanvasPreview(canvasIndex);
}
...
function deleteImage(canvasIndex, imageIndex) {
    ...
    if (canvasData[canvasIndex].selectedImageIndex >= canvasData[canvasIndex].images.length) {
        canvasData[canvasIndex].selectedImageIndex = canvasData[canvasIndex].images.length - 1;
    }
    updateImageThumbnails(canvasIndex);
    updateCanvasPreview(canvasIndex);
    updateTotalPrice();
...
function showCropModal(imageSrc, callback) {
    ...
    // Initialize cropper with 8x10 aspect ratio
    if (cropper) {
        cropper.destroy();
    }
...
function updateCanvasPreview(index) {
    ...
    // Draw background image (preview-bg.jpg)
...
function updatePreview() {
    updateCanvasPreview(currentCanvasIndex);
}
